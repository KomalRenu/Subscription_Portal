		...
		<TR>
			...
			<TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="3">
				<TR>
					<TD WIDTH="80%">
					<xsl:choose>
					<xsl:when test="./pa[@il='1' $or$ @idl='1']/mi/oi/es/e" >
					<!-- BEGIN: pulldown list -->	
					 	<SELECT>
					 			...
					 			<xsl:choose>
					 				<xsl:when test="./pa[@il='1' $or$ @idl='1']/mi/oi/es/e" >
					 					<xsl:for-each select="./pa[@il='1' $or$ @idl='1']/mi/oi/es/e" >
					     					<OPTION>
												<xsl:attribute name="VALUE"><xsl:value-of select="context()/@ei" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="context()/@disp_n" /></xsl:attribute>
					 					...
					 					<xsl:if test=".[not(@reqd)]">
					 						<OPTION>					 							
					 							<xsl:attribute name="VALUE"><xsl:value-of select="./pa[@il='1' $or$ @idl='1']/mi/oi/@did" />:0<xsl:eval no-entities="1">RS</xsl:eval><xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_2396").text</xsl:eval></xsl:attribute>
					 							<xsl:if test=".[@selected ='1']">
 													<xsl:attribute name="SELECTED">1</xsl:attribute>
 												</xsl:if>
					 							- <xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_512").text</xsl:eval> -</OPTION>
					 					</xsl:if>					 					
					 				</xsl:when>

					 				<xsl:otherwise>
					 					<OPTION VALUE="">--- <xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_512").text</xsl:eval> ---</OPTION>
					 				</xsl:otherwise>
					 			</xsl:choose>
					 	</SELECT>
					 	<!-- END: pulldown list -->	

...
<!-- #include file="addDBAlias_widget.asp" -->
<%
Dim lStatus
Dim aDBAliasInfo
Dim lRepositoryType
...
Const DUPLICATE_NAME = "DUPLICATE_NAME"
    ...
    Case REPOSITORY_MD:
        aPageInfo(S_TITLE_PAGE) = STEP_SELECT_MD_DBALIAS & " " & asDescriptors(621) 'Descriptor:Add a New Database Alias
        aPageInfo(N_CURRENT_OPTION_PAGE) = SECTION_ENGINE_CONFIG
        sRedirectPage = "select_md.asp"
    Case REPOSITORY_AUREP:
        aPageInfo(S_TITLE_PAGE) = STEP_SITE_AUREP_DBALIAS & " " & asDescriptors(621) 'Descriptor:Add a New Database Alias
        aPageInfo(N_CURRENT_OPTION_PAGE) = SECTION_PORTAL_MANAGEMENT
        sRedirectPage = "select_aurep.asp"
    Case REPOSITORY_SBREP:
        aPageInfo(S_TITLE_PAGE) = STEP_SITE_SBREP_DBALIAS & " " & asDescriptors(621) 'Descriptor:Add a New Database Alias
        aPageInfo(N_CURRENT_OPTION_PAGE) = SECTION_PORTAL_MANAGEMENT
	...
	For i = 0 to UBound(aDBAliases)
		If aDBAliases(i,0) = aDBAliasInfo(0) Then
			bDuplicate = True
			Exit For
		End If
	Next
	...
	If bDuplicate Then
		lErr = DUPLICATE_NAME
		sErrorMessage = Replace(asDescriptors(934), "#", aDBAliasInfo(0))
	Else
		'Proccess form submitted
		If oRequest("submit") <> 0 Then
			lErr = ProccessCreateDBAlias(aDBAliasInfo, lRepositoryType)
...
<HEAD>
  ...
  var sMsg
    ...
    sMsg = "";
    if (FormAddDBAlias.tAliasName.value == "" || isBlank(FormAddDBAlias.tAliasName.value)) {
      <%Call Response.Write("sMsg += ""<LI>" & asDescriptors(704) & """;") 'Descriptor:Please provide a name to the Alias Definition %>
    } else {
      if (checkIsAlphaNumeric(FormAddDBAlias.tAliasName.value) == false) <%Call Response.Write("sMsg += ""<LI>" & asDescriptors(882) & """;") 'Descriptor:Database connection names cannot contain special characters. Please enter a name which only uses a-z, 0-9 or underscore. %>
    }
    if (FormAddDBAlias.tServerName.value == "" || isBlank(FormAddDBAlias.tServerName.value)) <%Call Response.Write("sMsg += ""<LI>" & asDescriptors(703) & """;") 'Descriptor:Please provide a name to the Server Definition %>
    if (FormAddDBAlias.tDBName.value == "" || isBlank(FormAddDBAlias.tDBName.value)) <%Call Response.Write("sMsg += ""<LI>" & asDescriptors(705) & """;")'Descriptor:Please provide a name to the Database Definition %>
    if (FormAddDBAlias.tConfirmPassword.value != FormAddDBAlias.tPassword.value) <%Call Response.Write("sMsg += ""<LI>" & asDescriptors(276) & """;") 'Descriptor:Your second new password does not match the first. Please try again.%>
...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" HEIGHT="100%">
  ...
  <TR>
	...
	<TD WIDTH="96%" valign="TOP">
      <%If lErr <> NO_ERR And aDBAliasInfo(DBALIAS_CONFIRM) = False And lErr <> DUPLICATE_NAME Then
	  	Call DisplayAdminError(sErrorHeader, sErrorMessage, lErr, asDescriptors(250) & " " & asDescriptors(621), "addDBAlias.asp") 'Descriptor: Return to:'Descriptor:Subscription Engine Location"
	  Else%>
      ...
      <TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
        <FORM NAME="FormAddDBAlias" ACTION="addDBAlias.asp" METHOD="POST">
        ...
        <%End If%>
          ...
          <TR>
            <TD>
              <!--Start DBAlias list: -->
              <%
				lErr = displayAddDBAliasWidget(aDBAliasInfo, asDescriptors)
              %>
              <!--End DBAlias list -->
            </TD>
          </TR>
          ...
          <TR>
            <TD ALIGN="left" NOWRAP>
              <INPUT name=back   type=submit class="buttonClass" value="<%Response.Write(asDescriptors(120)) 'Descriptor:Cancel%>"></INPUT> &nbsp;
  			  <INPUT name=next   onClick="return validateForm();" type=submit class="buttonClass" value="<%Response.Write(asDescriptors(706)) 'Descriptor:Add database alias%>"></INPUT> &nbsp; <%'Need Descriptor:Add DB Alias%>

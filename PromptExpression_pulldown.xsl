				...
				<TR>
								...
								<xsl:when test="./res[. ='17' $or$ . ='18']" >
									<xsl:for-each select="./pa[@il='1' $or$ @idl='1']/mi/oi" >
										<xsl:for-each select="oi">	
											...
											<xsl:value-of select="context(-2)/@did" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./@did" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="context(-2)/@disp_n" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./@disp_n" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./@ddt" />
											</xsl:attribute>
											<xsl:attribute name="DATATYPE"><xsl:value-of select='@ddt' /> </xsl:attribute>
											<xsl:if test=".[@highlight='1']">
												<xsl:attribute name="SELECTED">1</xsl:attribute>
					...
					<xsl:attribute name="VALUE">
						...
						<xsl:when test="./pa[@ia='1']/exp/unknowndef" >	
							<!-- <xsl:value-of select="./pa[@ia='1']/exp/unknowndef/@text" /> -->
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>

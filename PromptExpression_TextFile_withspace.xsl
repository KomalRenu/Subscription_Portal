		...
		<TR>
			...
			<TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="3">
				<TR>
					...
					<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
					  <TR>
						...
						<TD ALIGN="LEFT" VALIGN="TOP">
							...
							<SELECT SIZE="10" MULTIPLE="1" STYLE="width: 250px;">
								...
								<xsl:if expr="this.selectSingleNode('/mi/inputs/DHTML').text=='1'">
									...
									<xsl:when test="./pa[@ia='1']/exp/nd/nd" >
										...
										<xsl:when test="./res[. = '10']" >
											...
											<!-- <xsl:when test=".[@optp = '1' $or$ $not$ @optp]" > -->
											<xsl:otherwise>
											...
											<OPTION>
												...
												<xsl:choose>
													<xsl:when test="./nd[1]/cst">
														<xsl:attribute name="VALUE"><xsl:value-of select="./nd[0]/oi[@tp='12']/@did" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./nd[0]/oi[@tp='21']/@did" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./nd[0]/@disp_n" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./nd[1]/cst/@ddt" /><xsl:eval no-entities="1">ESC</xsl:eval>M<xsl:value-of select="./op/@fnt" /><xsl:eval no-entities="1">ESC</xsl:eval>
														<xsl:for-each select="./nd[index()>0]"><xsl:eval>getNodeValue(this)</xsl:eval><xsl:if match="(./nd)[$not$ end()]">;</xsl:if>
														</xsl:for-each><xsl:eval no-entities="1">ESC</xsl:eval><xsl:value-of select="context(-2)/res" /><xsl:eval no-entities="1">ESC</xsl:eval><xsl:value-of select="./@disp_id" /></xsl:attribute><xsl:value-of select="./@disp_n" />
													</xsl:when>
													<xsl:otherwise>
														<xsl:attribute name="VALUE"><xsl:value-of select="./nd[0]/oi[@tp='12']/@did" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./nd[0]/oi[@tp='21']/@did" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./nd[0]/@disp_n" /><xsl:eval no-entities="1">RS</xsl:eval><xsl:value-of select="./nd[1]/@ddt" /><xsl:eval no-entities="1">ESC</xsl:eval>M<xsl:value-of select="./op/@fnt" /><xsl:eval no-entities="1">ESC</xsl:eval>
														<xsl:for-each select="./nd[index()>0]"><xsl:eval>getNodeValue(this)</xsl:eval><xsl:if match="(./nd)[$not$ end()]">;</xsl:if></xsl:for-each>
					...
					<xsl:choose>
						<xsl:when expr="this.selectSingleNode('/mi/inputs/DHTML').text=='1'">
							<FONT><xsl:attribute name="FACE"><xsl:eval>this.selectSingleNode("/mi/inputs/FontFamily").text</xsl:eval></xsl:attribute><xsl:attribute name="SIZE"><xsl:eval>this.selectSingleNode("/mi/inputs/smallFont").text</xsl:eval></xsl:attribute><xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_1381").text</xsl:eval><BR /></FONT>
							<INPUT TYPE="FILE" SIZE="30" CLASS="GOLDBUTTON" STYLE="font-family: courier">
								<xsl:attribute name="NAME">nuXML_TextFile_<xsl:value-of select='@pin' /></xsl:attribute>
								<xsl:attribute name="LABEL"><xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_1825").text</xsl:eval></xsl:attribute>								
								<xsl:if expr="this.selectSingleNode('/mi/inputs/DHTML').text=='1'">
									<xsl:attribute name="onFocus">selectInOperator(<xsl:value-of select='@pin' />);</xsl:attribute>
								</xsl:if>
							</INPUT>
							<IMG SRC="Images/ArrowDotted.gif" WIDTH="39" HEIGHT="22" ALT="" BORDER="0" />
							<INPUT TYPE="SUBMIT" CLASS="GOLDBUTTON">
								<xsl:attribute name="NAME">LoadFileGO_<xsl:value-of select='@pin' /></xsl:attribute>
								<xsl:attribute name="VALUE"><xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_1348").text</xsl:eval></xsl:attribute>
								<xsl:if expr="this.selectSingleNode('/mi/inputs/DHTML').text=='1'">
									<xsl:attribute name="onClick">selectInOperator(<xsl:value-of select='@pin' />); return(checkFileExtension(<xsl:value-of select='@pin' />));</xsl:attribute>
								</xsl:if>
							</INPUT>
						</xsl:when>
						<xsl:otherwise>
							<FONT><xsl:attribute name="FACE"><xsl:eval>this.selectSingleNode("/mi/inputs/FontFamily").text</xsl:eval></xsl:attribute><xsl:attribute name="SIZE"><xsl:eval>this.selectSingleNode("/mi/inputs/smallFont").text</xsl:eval></xsl:attribute><xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_1415").text</xsl:eval> <xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_1416").text</xsl:eval> <xsl:eval no-entities="1">this.selectSingleNode("/mi/inputs/Desc_1417").text</xsl:eval><BR /><BR /></FONT>
						</xsl:otherwise>
					</xsl:choose>
				</TD>
				</TR>
...
<xsl:script><![CDATA[
 ...
 function getNodeValue(e) {
	...
	var oCSTNode;
	if (e) {
		oCSTNode = e.selectSingleNode('cst');
		if (oCSTNode) { 
			if (oCSTNode.firstChild) {
				sValue = oCSTNode.firstChild.nodeValue;
			}
			else
				sValue = oCSTNode.text;
		}
		else
			sValue = e.text;
	}
